...
function buildFilterQuery(tableName, filters, pagination) {
    ...
    if (filters.ru_id) {
        // Handle multiple values for ru_id parameter
        if (Array.isArray(filters.ru_id)) {
            const ruIdConditions = [];
            for (const ruIdValue of filters.ru_id) {
                ruIdConditions.push(`(data->>'ru_id') ILIKE $${i++}`);
                values.push(`%${ruIdValue}%`);
            }
            where.push(`(${ruIdConditions.join(' OR ')})`);
        } else {
            where.push(`(data->>'ru_id') ILIKE $${i++}`);
            values.push(`%${filters.ru_id}%`);
        }
    }
